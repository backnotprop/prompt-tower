 ...
 */
export class MultiRootTreeProvider implements vscode.TreeDataProvider<FileNode> {
  private _onDidChangeTreeData = new vscode.EventEmitter<FileNode | undefined | void>();
  readonly onDidChangeTreeData = this._onDidChangeTreeData.event;
  
  private _onDidChangeSelection = new vscode.EventEmitter<FileSelectionChangeEvent>();
  ...
  private async initialize(): Promise<void> {
    ...
    try {
      ...
      console.error("MultiRootTreeProvider: Error during initialization:", error);
      vscode.window.showErrorMessage("Error initializing Prompt Tower file view.");
    }
  }
  ...
  async refreshWorkspaces(): Promise<void> {
    ...
    // Preserve currently checked paths
    const preserveCheckedPaths = new Set<string>();
    for (const checkedFile of FileNodeUtils.getCheckedFiles(this.rootNodes)) {
      preserveCheckedPaths.add(checkedFile.absolutePath);
    }
    ...
    this.rootNodes = await this.fileDiscoveryService.discoverFiles(workspaces, preserveCheckedPaths);
    ...
    console.log(`MultiRootTreeProvider: Refreshed ${this.rootNodes.length} workspace(s)`);
  }
  ...
  getTreeItem(element: FileNode): vscode.TreeItem {
    ...
    treeItem.checkboxState = element.checkable 
      ? (element.isChecked ? vscode.TreeItemCheckboxState.Checked : vscode.TreeItemCheckboxState.Unchecked)
      : undefined;
  ...
  findNodeByPath(absolutePath: string): FileNode | undefined {
    return this.fileDiscoveryService.findNodeByPath(this.rootNodes, absolutePath);
  }
