...
export class ContextGenerationService {
  ...
  private loadConfiguration(): void {
    ...
    this.config = {
      blockTemplate: outputFormat.blockTemplate || 
      ...
      wrapperTemplate: wrapperFormat === null ? null : 
      ...
      projectTree: {
        ...
        showFileSize: projectTreeFormat.showFileSize ?? true,
  ...
  async generateContext(
    ...
    if (fileCount === 0 && !hasSelectedIssues) {
      ...
      if (this.config.projectTree.enabled && 
        ...
        const fileTree = await this.generateProjectTree(fileNodes, options?.primaryWorkspaceRoot);
        let treeOnlyContext = this.config.projectTree.template.replace("{projectTree}", fileTree);
    ...
    try {
      ...
      const fileBlockPromises = fileCount > 0 ? 
  ...
  private async generateFileBlock(fileNode: FileNode): Promise<string> {
    try {
      ...
      formattedBlock = formattedBlock.replace(/{fileNameWithExtension}/g, fileNameWithExtension);
      ...
      formattedBlock = formattedBlock.replace(/{fullPath}/g, fileNode.absolutePath);
      ...
      formattedBlock = formattedBlock.replace(/{fileContent}/g, trimmedFileContent);
      
      return formattedBlock;
      
    } catch (error) {
      console.error(`Error generating block for file ${fileNode.absolutePath}:`, error);
   ...
   */
  private async generateProjectTree(fileNodes: FileNode[], primaryWorkspaceRoot?: string): Promise<string> {
    if (!this.config.projectTree.enabled) {
      return "";
    }
    ...
    const workspaceRoot = primaryWorkspaceRoot || this.determinePrimaryWorkspaceRoot(fileNodes);
    ...
    return generateFileStructureTree(
      ...
      {
        showFileSize: this.config.projectTree.type === "fullDirectoriesOnly" ? 
          false : this.config.projectTree.showFileSize
      }
    );
  }
   ...
   */
  private async getAllWorkspaceFiles(fileNodes: FileNode[]): Promise<StructuredFilePath[]> {
   ...
   */
  private collectFilesFromNode(node: FileNode, files: StructuredFilePath[]): void {
    if (node.type === 'file') {
      files.push({
        origin: node.absolutePath,
        tree: node.relativePath
      });
    } else if (node.type === 'directory' && this.config.projectTree.type === 'fullDirectoriesOnly') {
      files.push({
        origin: node.absolutePath + "/",
        tree: node.relativePath + "/"
      });
  ...
  async copyToClipboard(
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      vscode.window.showErrorMessage(`Error copying context to clipboard: ${errorMessage}`);
      throw error;
    }
  }
